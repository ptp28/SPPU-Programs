...
public class JDBCServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
		{
			...
			{
			case "INSERT" :
				ps = conn.prepareStatement("INSERT INTO `employeedata`(`name`, `salary`, `post`) VALUES (?,?,?);");
				...
				break;
			case "UPDATE" :
				ps = conn.prepareStatement("UPDATE `employeedata` SET `salary`=?,`post`=? WHERE name=?;");
				...
				break;
			case "DELETE" :
				ps = conn.prepareStatement("DELETE FROM `employeedata` WHERE name = ? ;");
				...
				break;
			case "DISPLAY" :
				ps = conn.prepareStatement("SELECT * FROM `employeedata` WHERE 1"); 
				rs = ps.executeQuery();
